...
function loadFirebaseData() {
    // Primeiro, carregue músicas públicas
    db.collection('songs')
        .where("isPublic", "==", true)
        .get()
        .then(publicSongsSnapshot => {
            // Processar músicas públicas
            publicSongsSnapshot.forEach(doc => {
            ...
            // Se o usuário estiver autenticado, carregue também suas músicas privadas
        ...
        .then(userSongsSnapshot => {
            // Processar músicas do usuário
            if (!userSongsSnapshot.empty) {
                userSongsSnapshot.forEach(doc => {
                    const song = { id: doc.id, ...doc.data() };
                    if (!songs.some(s => s.id === song.id)) {
                        songs.push(song);
                    }
