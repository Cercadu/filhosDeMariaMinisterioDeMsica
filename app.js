...
function loadFirebaseData() {
    // Carregar músicas públicas e do usuário atual
    const userId = AuthJS.getCurrentUserId();

    db.collection('songs')
        .where(firebase.firestore.Filter.or(
            firebase.firestore.Filter.where('isPublic', '==', true),
            firebase.firestore.Filter.where('userId', '==', userId)
        ))
        .get()
        .then(snapshot => {
            allSongs = [];
        ...
        .catch(error => {
            ...
            showErrorToast('Erro ao carregar músicas do servidor');
        });
    ...
    db.collection('playlists')
        ...
        .get()
        .then(snapshot => {
            // Manter apenas playlists locais
            personalPlaylists = personalPlaylists.filter(p => p.id.startsWith('local_'));

            // Adicionar playlists do Firebase
            snapshot.forEach(doc => {
